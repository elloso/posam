...
class Position extends Admin_Controller 
{
	...
	//--> Redirects to the manage position page
	...
	public function fetchPositionDataById($position_id) 
	{
		if($position_id) {
			$data = $this->model_position->getPositionData($position_id);
	...
	public function fetchActivePosition() 
	{
		$data = $this->model_position->getActivePosition();
		echo json_encode($data);
	...
	public function fetchPositionData()
	{
		...
		$data = $this->model_position->getPositionData(); 
	...
	public function update($position_id)
	{
		...
		if($position_id) {
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	...
	        	$update = $this->model_position->update($data, $position_id);
	        	if($update == true) {
	        		$response['success'] = true;
	...
	public function remove()
	{
		...
		if($position_id) {
			...
			$total_used = $this->model_position->checkIntegrity($position_id);
			//---> If no table has this information, we can delete
            if ($total_used == 0) {        
				$delete = $this->model_position->remove($position_id);
				if($delete == true) {
					$response['success'] = true;
