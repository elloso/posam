...
{	 
	public function __construct()
	{
		...
		$this->data['page_title'] = 'Report08 - Order Slip';
	...
	$REP08 = $this->model_report->get_REP08();
	...
	if ($REP08 == null) {
			...
			$report = $this->model_report->getReportOrderItem($cs->order_id);
			...
			$this->table->add_row($page_break);  


			endforeach;  //ALL order_slip



			//---> Print the total by item for the area
			$area_name = '';
			$cell1 = array('data' => '<h2>Summary of Orders</h2>');	
			$this->table->add_row($cell1);

			$area = $this->model_report->getReport08TotalItemByArea();
			if ($area == null) {}
				else {
				foreach ($area as $ar):
					   
						if ($ar->area_name <> $area_name) {
							$area_name = $ar->area_name;
							$cell1 = array('data' => '');	
							$this->table->add_row($cell1);
						} else {$area_name = '';}

						$cell1 = array('data' => '<strong>'.$area_name.'</strong>', 'width' => '15%');
						$cell2 = array('data' => '<strong>'.$ar->quantity.'</strong>', 'width' => '12%', 'align' => 'right');
					    $cell3 = array('data' => $ar->item_name, 'width' => '40%');
						$this->table->add_row($cell1, $cell2, $cell3);
						$area_name = $ar->area_name;
				endforeach;
			}
